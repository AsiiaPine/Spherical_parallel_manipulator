...
from animate_rotatioin import normalize, rot_matrix_quaternion
...
def draw_rotation(
    ...
    quiverx = ax.quiver(X=[0], Y=[0], Z=[0],
                        U=x[0], V=y[0], W=z[0],
                        length=1, colors=colors[0], normalize=True, arrow_length_ratio=0.3)
    quivery = ax.quiver(X=[0], Y=[0], Z=[0],
                        U=x[1], V=y[1], W=z[1],
                        length=1, colors=colors[1], normalize=True, arrow_length_ratio=0.3)
    quiverz = ax.quiver(X=[0], Y=[0], Z=[0],
                        U=x[2], V=y[2], W=z[2],
                        length=1, colors=colors[2], normalize=True, arrow_length_ratio=0.3)
    fig.canvas.draw()
    fig.canvas.flush_events()
